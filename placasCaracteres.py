...
from reconocimientoCaracteres import *
...
try:
    ...
    # # El índice 0 se refiere a la cámara incorporada
    cap = cv2.VideoCapture(0)
    
    # Obtener las dimensiones del cuadro capturado
    frame_width = int(cap.get(3))  # Ancho del cuadro
    frame_height = int(cap.get(4))  # Alto del cuadro
    ...
    try:
        while(cap.isOpened()):
            ...
            if contador_frames >= frecuencia_deteccion:
               # Realiza la detección de la placa cada 10 frames
               placa = detectarPlaca(frame)
               # Restablece el contador de frames
               contador_frames = 0
