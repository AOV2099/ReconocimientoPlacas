        ...
        for j in posiblesEtiq:
            if os.path.isfile(j + '-' + str(i) + ".jpg"):
                ...
                if img is not None:
                    ...
                    datos.append(features.reshape(-1, 1))  # Cambia la forma a (144, 1)
...
# datos, etiquetas = obtenerDatos()
# #-------------------------- Evaluacion KNN
# X_train,X_test,y_train,y_test=train_test_split(datos,etiquetas,test_size=0.2, random_state=42) #el 42 es arbitrario
# knn=KNeighborsClassifier(n_neighbors=1)
